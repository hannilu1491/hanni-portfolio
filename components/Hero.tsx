...
export default function Hero() {
  ...
  const layer = useMemo(
    () => ({
      main: { transform: `translate3d(${mouse.x * 0.16}px, ${mouse.y * 0.16 - scrollY * 0.12}px,0)` },
      cardA: { transform: `translate3d(${mouse.x * 0.34}px, ${mouse.y * 0.34 - scrollY * 0.22}px,0)` },
      cardB: { transform: `translate3d(${mouse.x * 0.46}px, ${mouse.y * 0.46 - scrollY * 0.3}px,0)` }
    }),
    [mouse, scrollY]
  );
    ...
    <section className="fade-in bg-[#fcfbf8]">
      <div className="mx-auto grid max-w-7xl gap-10 px-6 pb-20 pt-16 md:grid-cols-[45%_55%] md:items-center">
        <div className="space-y-7">
          ...
          <p className="max-w-xl text-[1.05rem] leading-8 text-warm-700">Premium editorial digital experiences with calm visual rhythm and refined storytelling.</p>
        ...
        <div className="relative h-[620px] overflow-hidden rounded-[36px] md:h-[680px]">
          ...
          <div style={layer.main} className="absolute left-[9%] top-[14%] w-[82%] rounded-[28px] border border-[#e7ddd0] bg-white/70 p-3 shadow-[0_30px_64px_rgba(32,24,18,0.2)] backdrop-blur-[1px]">
            <div className="relative h-[390px] overflow-hidden rounded-[20px] md:h-[430px]">
              <Image src={mainVisual} alt="" fill className="object-cover" />
            </div>
          </div>

          <div style={layer.cardA} className="float-soft absolute right-[9%] top-[12%] w-[180px] rounded-2xl border border-[#eadfce] bg-white/88 p-4 shadow-soft">
          ...
          <div style={layer.cardB} className="float-soft-delay absolute bottom-[12%] left-[7%] w-[170px] rounded-2xl border border-[#eadfce] bg-white/88 p-4 shadow-soft">
