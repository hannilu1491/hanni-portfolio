...
export default function Detail({ params }: { params: { slug: string } }) {
  ...
  return (
    <main className="bg-[#fcfbf8] pb-16">
      ...
      <CaseStudySection title="Visual System 視覺系統"><VisualSystemSection text={p.visualSystem || 'Typography, color, and layout system tuned for editorial rhythm.'} /><div className="mt-6"><EditorialGallery images={p.galleryImages.slice(0, Math.max(1, Math.min(3, p.galleryImages.length)))} /></div></CaseStudySection>
      <CaseStudySection title="Process / Exploration 設計流程"><ProcessGrid process={p.process || 'Sketch → Wireframe → Visual Direction → Iteration → Final UI'} /><div className="mt-6"><MockupSection images={p.galleryImages.slice(0, 2)} /></div></CaseStudySection>
      ...
      <CaseStudySection title="Responsive Design 響應式"><EditorialGallery images={[...p.galleryImages].reverse()} /></CaseStudySection>
